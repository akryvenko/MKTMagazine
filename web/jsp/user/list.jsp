...
<div class="buttonBar">
    <span class="menuButton"><a class="create"
                                href="<c:url value="/customer.do"><c:param name="action" value="create"/></c:url>">Новий
        Замовник</a></span>
</div>
<div class="body">
    <h1>Список Користувачiв</h1>
    ...
    <div class="list">
        <table>
            ...
            <c:forEach var="user" items="${users}" varStatus="i">
                <tr>
                    <td>
                        <a href="<c:url value="/customer.do"><c:param name="customerId" value="${customer.id}"/><c:param name="action" value="edit"/></c:url>"><c:out
                                value="${customer.id}"/></a>
                    </td>
                    <td>
