...
public class PeriodReportListReport {

    private File reportFile;


    public PeriodReportListReport(File filepath){
        reportFile = filepath;
	}
    ...
    public void generate(Office office, Employer employee, Date startDate, Date endDate, Collection<Order> employees) throws Exception {
        ...
        document.add(getTextParagraph("Список оформлених замовлень на " + employee.getFullName() + " з " + format.format(startDate) + " по " + format.format(endDate)));
        ...
        for (Order order : employees) {
            ...
            if (order.getStatus() == OrderStatus.InProgress.getStatusValue()) {
                ...
                table.addCell(createCell("Скасований" , BaseColor.WHITE, Element.ALIGN_LEFT));
            }
